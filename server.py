...
async def get_voice_config():
    """Public config for the ElevenLabs voice widget — only the agent id, never the API key."""
    return {"agent_id": ELEVENLABS_AGENT_ID, "voice_enabled": bool(ELEVENLABS_AGENT_ID)}
...
async def chat(payload: ChatRequest):
    ...
    Returns: { session_id, messages: [{type, content, sender}],
               protocol_messages: [{timestamp, sender, receiver, type, summary, details}] }
    protocol_messages holds only the agent-to-agent wire messages generated by THIS
    turn (catalog lookups, negotiation rounds, payment events) — drives the live
    agent-network panel without re-sending the whole session's history each call.
    """
    session = conversation_agent.get_or_create_session(payload.session_id)
    protocol_start = len(session.protocol_messages)
    messages = await conversation_agent.handle_message(session, payload.message)
    new_protocol_messages = session.protocol_messages[protocol_start:]
    return {
        "session_id": session.session_id,
        "messages": messages,
        "protocol_messages": new_protocol_messages,
    }
...
async def get_chat_history(session_id: str):
    ...
    return {
        ...
        "protocol_messages": session.protocol_messages,
    }
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*60)
    print("  Hermes Protocol — AI Agent-to-Agent Commerce")
